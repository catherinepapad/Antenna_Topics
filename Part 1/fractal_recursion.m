function antenna_coord=fractal_recursion(x1,y1,x2,y2,it,angle)
...
c = [b(2) b(2)+length  b(4) b(4)];          %coordinates of center-left horizontal segment
f = [x4+hor x4  y4-height  y4];             %coordinates of center diagonal segment
e = [f(1)-length f(1) f(3) f(3)];           %coordinates of center-right horizontal segment
...
if(it-1>0)                          %check for recursion
    ...
    antenna_coord(3,:) = [];
    antenna_coord(1,:) = [];
    antenna_coord = [antenna_coord;fractal_recursion(a(1),a(3),a(2),a(4),it-1,angle)];      %create the new segments  
    antenna_coord = [antenna_coord;fractal_recursion(c(1),c(3),c(2),c(4),it-1,angle)];
    antenna_coord = [antenna_coord;fractal_recursion(e(1),e(3),e(2),e(4),it-1,angle)];
    antenna_coord = [antenna_coord;fractal_recursion(g(1),g(3),g(2),g(4),it-1,angle)];
end
